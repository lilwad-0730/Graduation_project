...
public class StairsCameraZoom : MonoBehaviour
{
    ...
    private float _lastZoomPlayerY;
    ...
    void Start()
    {
        ...
        _lastZoomPlayerY = transform.position.y;
    ...
    private void DetectStairs()
    {
        ...
        if (hitStair)
        {
            ...
            // 如果正在往下跳或往下走（Y軸高度明顯降低超過 0.2f），主動還原鏡頭
            if (transform.position.y < _lastZoomPlayerY - 0.2f && !isClimbingUp)
            {
                RestoreCamera();
                return;
            }
            ...
            if (isClimbingUp)
            {
                if (stackByGameObject)
                {
                    ...
                    if (hitStairObj != _lastStairsObject)
                    {
                        ...
                        _lastZoomPlayerY = transform.position.y; // 更新高度參考點
                        ApplyZoomOut();
                    }
                }
                else
                {
                    ...
                    {
                        _lastStairsObject = hitStairObj;
                        _stairStartHeight = transform.position.y;
                        _lastZoomPlayerY = transform.position.y; // 更新高度參考點
                        ApplyZoomOut();
                    }
                    else
                    {
                        float heightDiff = transform.position.y - _stairStartHeight;
                        if (heightDiff > 0f)
                        {
    ...
    private void RestoreCamera()
    {
        ...
        _waitingToRestore = false;
        _lastZoomPlayerY = transform.position.y; // 重置參考高度
