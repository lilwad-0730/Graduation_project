...
public class PlayerShield : MonoBehaviour, IResettable
{
    ...
    [Header("關卡限制 (Allowed Scenes)")]
    [Tooltip("允許使用護盾的關卡名稱關鍵字 (例如 desert 或 荒漠；其他關卡如 玻璃管/天空/水下 按 Q 均不觸發且不生成多餘物件)")]
    ...
    public bool IsShieldAllowedInCurrentScene()
    {
        ...
        {
            if (currentScene.Contains(keyword.ToLower())) return true;
        }
        return false;
    }
    ...
    private void EnsureShieldObject()
    {
        ...
        // 3. 備用方案：自動生成一個發光護盾球體 (僅在荒原關卡且無 Shield 子物件時)
